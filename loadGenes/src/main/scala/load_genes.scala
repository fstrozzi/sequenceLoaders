...
object Main extends App {
  ...
  entries.foreach {e =>
    ...
    if(seq.size >= 1000) {
      sequences.append(Seq(seq_id,seq_description,seq,sample_id,prokka_id))
    }
  ...
  def DBcommit(sequences: scala.collection.mutable.ListBuffer[Seq[String]]) {
    DB autoCommit { implicit session =>
      sql"insert into genes (gene_id, description, nucleotide_sequence, sample_id, prokka_id) values (?,?,?,?,?)".batch(sequences: _*).apply()
    }
  }
}
